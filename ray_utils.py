...
def get_pixels_from_image(image_size, camera):
    ...
    # (Q1.3): Generate pixel coordinates from [0, W] in x and [0, H] in y
    x = torch.arange(0, W)
    y = torch.arange(0, H)

    # (Q1.3): Convert to the range [-1, 1] in both x and y
    x = 2 * ((x / W) - 0.5)
    y = 2 * ((y / H) - 0.5)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    # (Q2.1): Random subsampling of pixel coordinaters
    idxs = torch.randperm(image_size[0] * image_size[1])[:n_pixels]
    xy_grid_sub = xy_grid[idxs].to("cuda")
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    # (Q1.3): Use camera.unproject to get world space points from NDC space points
    image_plane_points = camera.unproject_points(ndc_points, world_coordinates=True, from_ndc=True)

    # (Q1.3): Get ray origins from camera center
    rays_o = camera.get_camera_center().repeat(image_plane_points.shape[0], 1)

    # (Q1.3): Get ray directions as image_plane_points - rays_o
